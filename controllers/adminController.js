...
const creditUserWallet = asyncHandler(async (req, res) => {
  ...
  const transactionId =
    username + `${currentYear}${currentMonth}${currentHour}${currentMinutes}`;
...
const viewQualifiedUsers = asyncHandler(async (req, res) => {
  try {
    const qualifiedUsers = await QualifiedUser.find()
      .sort({ createdAt: -1 })
      .populate("user", "username")
      .exec();
...
const editUserPersonalInformation = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      user.withdrawableCommission =
        withdrawableCommission || user.withdrawableCommission;
...
const editUserBankDetails = asyncHandler(async (req, res) => {
  const { username, accountName, accountNo, bankName } = req.body;
...
const editUsername = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      const upline = user.upline.ID
        ? await User.findById(user.upline.ID)
        : null;
...
const getTotalAdmin = asyncHandler(async (req, res) => {
